...
class CalibBlock:
    ...
    def __init__(self, blockName: str, yamlBlock: Mapping[str, Any]):
        self.name = blockName
        self.sources = {}

        for typeName, block in yamlBlock.items():
            self.sources[typeName] = CalibSource.getSubclass(typeName).fromYaml(block)
...
class ScienceBlock:
    ...
    def __init__(self, blockName: str, yamlBlock: Mapping[str, Any]):
        self.name = blockName
        self.source = SourceFilter.fromYaml(yamlBlock)

        self.data = {}
        for step in self.steps:
            block = yamlBlock.get("policy", {}).get(step, {})
            self.data[step] = ScienceStep.getSubclass(step).fromYaml(block)
...
def processYaml(yamlFile: str) -> Tuple[InitSource, Dict[str, CalibBlock], Dict[str, ScienceBlock]]:
    ...
    calibBlocks = {}
    for blockName, yamlBlock in content.get("calibBlocks", {}).items():
        calibBlocks[blockName] = CalibBlock(blockName, yamlBlock)

    scienceBlocks = {}
    for blockName, yamlBlock in content.get("scienceBlocks", {}).items():
        scienceBlocks[blockName] = ScienceBlock(blockName, yamlBlock)
