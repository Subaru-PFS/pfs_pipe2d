...
# Science pipeline
pipetask run --register-dataset-types -j $CORES -b $DATASTORE --instrument lsst.obs.pfs.PfsSimulator -i PFS-F/raw/sps,PFS-F/raw/pfsConfig,PFS-F/calib,PFS-F/objectGroups -o "$RERUN"/science -p '$DRP_STELLA_DIR/pipelines/science.yaml' -d "combination = 'object'" --fail-fast -c isr:doCrosstalk=False -c fitFluxCal:fitFocalPlane.polyOrder=0 -c fitFluxCal:fluxRefMinReducedChi2=-1 -c fitFluxCal:fluxRefMaxReducedChi2=-1 -c reduceExposure:doApplyScreenResponse=False -c reduceExposure:doBlackSpotCorrection=False -c reduceExposure:doScatteredLight=False -c reduceExposure:doApplyPfiCorrection=False -c 'reduceExposure:targetType=[SCIENCE, SKY, FLUXSTD]' -c cosmicray:grouping=separate
