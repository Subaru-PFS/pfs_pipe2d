...
pipetask run --register-dataset-types -j $CORES -b $DATASTORE --instrument lsst.obs.pfs.PfsSimulator -i PFS-F/raw/sps,PFS-F/raw/pfsConfig,PFS-F/calib -o "$RERUN"/skyNorms -p '$DRP_STELLA_DIR/pipelines/skyNorms.yaml' -d "instrument='PFS-F' AND combination = 'object'" -c isr:doCrosstalk=False -c reduceExposure:doApplyScreenResponse=False -c reduceExposure:doBlackSpotCorrection=False -c reduceExposure:doScatteredLight=False -c reduceExposure:doApplyPfiCorrection=False -c skyNorms:fitFocalPlane.order=0 --fail-fast
