...
usage() {
    ...
    echo "" 1>&2
    echo "Usage: $0 [-f FIBERS] [-a ARMS] [-j NUMCORES] [-n] [-d OBJ_SPECTRA_DIR]" 1>&2
    echo "" 1>&2
    echo "    -f <FIBERS> : fibers to activate (all,lam,...)" 1>&2
    echo "    -a <ARMS> : arms to illuminate (b, r, n, m, br, brn..)" 1>&2
...
FIBERS=all
ARMS=brn
while getopts "hf:a:j:d:n" opt; do
    case "${opt}" in
        f)
            FIBERS=${OPTARG}
            ;;
